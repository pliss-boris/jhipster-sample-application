...
@RestController
@RequestMapping("/api/jhdemo-kafka")
public class JhdemoKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JhdemoKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JhdemoKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
